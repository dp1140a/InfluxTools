...
influx \
	...
	-database '_internal' \
	-execute 'SELECT last(numMeasurements) as last_measurement, last(numSeries) as last_series FROM "_internal"."monitor"."database" WHERE time > now() - 24h GROUP BY time(1h),"database","hostname" fill(0)' \
	> $OUT_DIR/countsSeriesAndMeasurements.txt
